...
visualizeGame <- function(id, var){
  ...
  if (var == "Odds") m1 <- mPlot(x="q", y=c("left.prob", "center.prob", "right.prob"), type="Line", data=game, labels=players, pointSize=0, lineWidth=2, parseTime=F, postUnits="%", hoverCallback=hoverFunction, ymin=0, ymax=100)
  return(m1)
}