...
# Fit model
gbm.model <- gbm(factor(winner.rank) ~ middle.diff + middle.ratio + bottom.diff + bottom.ratio + money.left + dd.remaining, data=usable.points[!j.archive.id %in% test.ids], distribution="multinomial", shrinkage=0.01, n.trees=1500, verbose=T)

# Predict
preds <- predict(gbm.model, usable.points[j.archive.id %in% test.ids], n.trees=800, type="response")[, , 1]
